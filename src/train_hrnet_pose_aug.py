...
# Run utils
# -----------------------------------------------------------------------------
SCRIPT_SIGNATURE = "train_hrnet_pose_aug.py :: v1 :: domain-shift albumentations + normalized coord loss"
...
class AugmentedSpeedPlusDataset(SpeedPlusKeypointDataset):
    """
    Apply albumentations to image + 2D keypoints, then rebuild heatmaps.
    This is critical: supervision must match the augmented keypoints.
    """

    def __init__(self, *args, aug: Optional[A.Compose] = None, **kwargs):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        x = img_aug.astype(np.float32) / 255.0
        out["image"] = torch.from_numpy(x).permute(2, 0, 1).contiguous()
        out["kpts_2d"] = torch.from_numpy(kpts_aug)

        # Rebuild GT heatmaps from augmented kpts
        if self.return_heatmaps:
            out["heatmaps"] = self._make_heatmaps(kpts_aug)
...
def main():
    ...
    val_ds = AugmentedSpeedPlusDataset(
        ...
        sigma=cfg.sigma,
        return_heatmaps=True,
        aug=None,  # IMPORTANT: no aug on val
    )
