...
def _reproj_err_per_point_px(pts3d: np.ndarray, pts2d: np.ndarray, K: np.ndarray, R: np.ndarray, t: np.ndarray) -> np.ndarray:
    proj = reprojection_points(pts3d, K, R, t)  # (N,2)
    d = proj.astype(np.float32) - pts2d.astype(np.float32)
    return np.sqrt(np.sum(d * d, axis=1))  # (N,)


def refine_pose_lm_opencv(
    pts3d: np.ndarray,
    pts2d: np.ndarray,
    K: np.ndarray,
    R0: np.ndarray,
    ...
    """
    LM refinement using OpenCV. Returns (ok, R, t).
    """
    try:
        ...
        return True, R.astype(np.float32), t
    except cv2.error:
        return False, R0, t0


def refine_pose_lm_retrim(
    ...
    """
    2-pass: LM -> retrim best points -> LM
    """
    info = {"pass1_ok": False, "pass2_ok": False,
            "n0": int(pts3d_all.shape[0]), "n_keep": None}

    ok1, R1, t1 = refine_pose_lm_opencv(pts3d_all, pts2d_all, K, R0, t0)
    info["pass1_ok"] = bool(ok1)
    if not ok1:
        return False, R0, t0, info

    err = _reproj_err_per_point_px(pts3d_all, pts2d_all, K, R1, t1)
    n = int(err.size)
    k = int(max(min_keep, np.floor(keep_frac * n)))
    k = min(k, n)

    idx = np.argsort(err)[:k]
    info["n_keep"] = int(k)

    ok2, R2, t2 = refine_pose_lm_opencv(
        pts3d_all[idx], pts2d_all[idx], K, R1, t1)
    info["pass2_ok"] = bool(ok2)

    if ok2:
        return True, R2, t2, info
    return True, R1, t1, info  # pass1 ok, pass2 failed -> keep pass1
...
class EvalCfg:
    ...
    refine_enable: int = 1
    refine_retrim: int = 1
    refine_keep_frac: float = 0.8   # tieni 80% dei migliori
    refine_min_points: int = 6      # minimo punti per fare LM
    refine_min_keep: int = 6        # minimo dopo retrim
...
def parse_args() -> EvalCfg:
    ...
    # LM refinement
    ap.add_argument("--refine_enable", type=int, default=1)
    ap.add_argument("--refine_retrim", type=int, default=1)
    ap.add_argument("--refine_keep_frac", type=float, default=0.8)
    ap.add_argument("--refine_min_points", type=int, default=6)
    ap.add_argument("--refine_min_keep", type=int, default=6)
    ...
    return EvalCfg(
        ...
        refine_enable=int(a.refine_enable),
        refine_retrim=int(a.refine_retrim),
        refine_keep_frac=float(a.refine_keep_frac),
        refine_min_points=int(a.refine_min_points),
        refine_min_keep=int(a.refine_min_keep),
...
def main():
    ...
    for i in tqdm(range(n_eval), desc=f"eval {cfg.domain}/{cfg.split}"):
        ...
        refine_ok = False
        refine_info = None
        R_use, t_use = R_CT_est, t_est  # default: base pose

        if cfg.refine_enable and ninl >= cfg.refine_min_points:
            pts3d_inl = kpts_3d_all[idx_inl]
            pts2d_inl = kpts_2d_pred_crop[idx_inl]

            if cfg.refine_retrim:
                refine_ok, Rr, tr, refine_info = refine_pose_lm_retrim(
                    pts3d_inl, pts2d_inl, K_crop,
                    R_CT_est, t_est,
                    keep_frac=float(cfg.refine_keep_frac),
                    min_keep=int(cfg.refine_min_keep),
                )
            else:
                refine_ok, Rr, tr = refine_pose_lm_opencv(
                    pts3d_inl, pts2d_inl, K_crop, R_CT_est, t_est)
                refine_info = {"pass1_ok": bool(refine_ok), "pass2_ok": None, "n0": int(
                    pts3d_inl.shape[0]), "n_keep": None}

            # Safety: mantieni solo se t_z positiva (evita flip assurdi)
            used_refined = False
            if refine_ok and float(tr[2]) > 0.0 and is_finite_arr(tr) and is_finite_arr(Rr):
                R_use, t_use = Rr, tr
                used_refined = True

                # dopo aver deciso R_use, t_use
                t_use_norm = float(np.linalg.norm(t_use))
                t_ratio_use = float(
                    t_use_norm / (np.linalg.norm(t_gt) + 1e-12))

                if (not cfg.disable_t_ratio_gate) and (t_ratio_use > cfg.t_ratio_max):
                    # rollback: non scarto il sample, ma evito outlier post-refine
                    R_use, t_use = R_CT_est, t_est
                    used_refined = False
                    if isinstance(refine_info, dict):
                        refine_info["rolled_back"] = True
                        refine_info["t_ratio_use"] = float(t_ratio_use)

        # Quaternion from estimated R (SciPy xyzw)
        q_est_xyzw = SciRot.from_matrix(R_use).as_quat().astype(np.float32)
        q_est_wxyz = np.array(
            [q_est_xyzw[3], q_est_xyzw[0], q_est_xyzw[1], q_est_xyzw[2]], dtype=np.float32)

        # For 2D metrics: project with K_full into FULL coords
        proj_full = reprojection_points(kpts_3d_all, K_full, R_use, t_use)

        rmse_vs_gt = float("nan")
        m = np.isfinite(kpts_2d_gt_full).all(axis=1) & (
            kpts_2d_gt_full[:, 0] >= 0) & (kpts_2d_gt_full[:, 1] >= 0)
        if np.count_nonzero(m) >= 2:
            rmse_vs_gt = reprojection_rmse_px(
                kpts_2d_pred_full[m], kpts_2d_gt_full[m])

        # Pose metrics
        Et_i = float(np.linalg.norm(t_gt - t_use))
        ...
        _, _, sc_pos, sc_ori, sc = esa_score_components(
            t_gt, t_use, q_gt_wxyz, q_est_wxyz)
