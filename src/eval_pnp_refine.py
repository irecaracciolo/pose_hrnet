...
class SA_LMPE:
    """
    Simulated Annealing Levenberg-Marquardt Pose Estimation
    Refines pose estimates by iteratively optimizing with robust loss.
    """

    def __init__(self, camera_matrix):
        self.K = camera_matrix

    def project_points(self, X_3d, rvec, tvec):
        x_2d, _ = cv2.projectPoints(
            X_3d.reshape(-1, 1, 3),
            rvec.reshape(3, 1),
            tvec.reshape(3, 1),
            self.K,
        ...
        return x_2d.reshape(-1, 2)

    def compute_reprojection_errors(self, pose, X_3d, x_2d):
        rvec, tvec = pose[:3], pose[3:6]
        x_proj = self.project_points(X_3d, rvec, tvec)
        errors = np.linalg.norm(x_2d - x_proj, axis=1)
        return errors

    def residuals_huber(self, pose, X_3d, x_2d, delta):
        rvec, tvec = pose[:3], pose[3:6]
        x_proj = self.project_points(X_3d, rvec, tvec)
        residuals = (x_2d - x_proj).ravel()
        residuals_2d = residuals.reshape(-1, 2)
        r_norms = np.linalg.norm(residuals_2d, axis=1)
        weights = np.where(r_norms <= delta, 1.0, delta / (r_norms + 1e-8))
        weights_2d = np.repeat(weights, 2)
        return residuals * np.sqrt(weights_2d)

    def refine_pose(self, rvec_init, tvec_init, keypoints_3d, keypoints_2d,
                    delta_init=5.0, eps_init=50.0, delta_min=1.0, eps_min=4.0,
                    lambda_delta=0.7, lambda_eps=0.7, max_iters=10, verbose=False):
        min_points = 6
        pose = np.concatenate([rvec_init.ravel(), tvec_init.ravel()])
        inlier_indices = np.arange(len(keypoints_3d))
        delta = delta_init
        eps = eps_init
        for iteration in range(max_iters):
            X_3d_active = keypoints_3d[inlier_indices]
            x_2d_active = keypoints_2d[inlier_indices]
            if len(X_3d_active) < min_points:
                break

            result = least_squares(
                fun=self.residuals_huber,
                x0=pose,
                args=(X_3d_active, x_2d_active, delta),
                method='lm',
                verbose=0
            )
            pose = result.x
            errors = self.compute_reprojection_errors(
                pose, keypoints_3d[inlier_indices], keypoints_2d[inlier_indices]
            )
            inlier_mask = errors <= eps
            inlier_indices = inlier_indices[inlier_mask]
            delta = max(delta_min, lambda_delta * delta)
            eps = max(eps_min, lambda_eps * eps)
            if len(inlier_indices) < 4:
                break

        final_inliers_mask = np.zeros(len(keypoints_3d), dtype=bool)
        final_inliers_mask[inlier_indices] = True
        return pose[:3], pose[3:6], final_inliers_mask
...
def main():
    ...
    for i in tqdm(range(n_eval), desc=f"eval {cfg.domain}/{cfg.split}"):
        ...
        R_CT_est, t_est, inliers = refiner.refine_pose(
            R_CT_est, t_est, kpts_3d_all, kpts_2d_pred_all
        )
        R_CT_est, _ = cv2.Rodrigues(R_CT_est)
