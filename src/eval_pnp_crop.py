    ...
    for i in tqdm(range(n_eval), desc=f"eval {cfg.domain}/{cfg.split}"):
        ...
        if use_crop:
            if (kpts_2d_gt_full is None) or (not np.all(np.isfinite(kpts_2d_gt_full))) or (not np.any(kpts_2d_gt_full[:, 0] >= 0)):
                # Can't compute GT bbox -> fallback to full (but count it)
                ...
                if bbox is None:
                    ...
                    # Crop+resize image ONLY (no need to transform GT here; we keep GT in full coords for metrics)
                    img_in_rgb, _, tfm = crop_and_resize(
                        img_full_rgb,
                        kpts_2d_gt_full,   # dummy; returned pts not used here
                        bbox,
                        out_wh=(Wout, Hout),
                    )
        ...
        # kpts pred in CROPPED resized coords (i.e. coords of the image fed to model)
        ...
        # Reprojection on inliers gate (CROP coords, consistent with solvePnPRansac)
        proj_crop_all = reprojection_points(
            kpts_3d_all, K_crop, R_CT_est, t_est)  # (K,2) in crop coords
